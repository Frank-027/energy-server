...
def batterij_diagnose_dag(verbinding, datum):
    ...
    for i in range(1, len(rijen)):
        ...
        soc_begin = vorige["battery_percentage"] 
        soc_einde = huidige["battery_percentage"] 
        opslag_begin = vorige["full_pack_energy_available_wh"] 
        opslag_einde = huidige["full_pack_energy_available_wh"] 
        power = vorige["power_w"] 
        charged_begin = vorige["lifetime_energy_charged_wh"] 
        charged_einde = huidige["lifetime_energy_charged_wh"] 
        discharged_begin = vorige["lifetime_energy_discharged_wh"] 
        discharged_einde = huidige["lifetime_energy_discharged_wh"]

        if ( 
            opslag_begin is not None 
            and opslag_einde is not None 
        ): 
            delta_opslag_kwh = ( 
                opslag_einde - opslag_begin 
            ) / 1000 
        else: 
            delta_opslag_kwh = 0.0
        ...
        delta_charged_kwh = ( 
            charged_einde - charged_begin
        ) / 1000 

        delta_discharged_kwh = ( 
            discharged_einde - discharged_begin 
        ) / 1000

        if power > 0: 
            aantal_laden += 1 
            totaal_power_laden += energie_power_kwh 
            totaal_lifetime_charged += delta_charged_kwh 

            # Alleen positieve opslagverandering telt als # energie die effectief in de batterij terechtkomt. 
            if delta_opslag_kwh > 0: 
                totaal_soc_opslag_laden += delta_opslag_kwh
        elif power < 0: 
            aantal_ontladen += 1 
            # Voor de samenvatting maken we ontladen positief. totaal_power_ontladen += abs(energie_power_kwh) 
            totaal_lifetime_discharged += delta_discharged_kwh 
            # Alleen negatieve opslagverandering telt als # energie die effectief uit de batterij verdwijnt. 
            if delta_opslag_kwh < 0: 
                totaal_soc_opslag_ontladen += abs(delta_opslag_kwh)

        print( 
            f"{huidige['timestamp']} " 
            f"{soc_begin:7.2f} " 
            f"{soc_einde:7.2f} " 
            f"{opslag_begin / 1000:9.3f} " 
            f"{opslag_einde / 1000:9.3f} " 
            f"{delta_opslag_kwh:9.3f} " 
            f"{power:9.1f} " 
